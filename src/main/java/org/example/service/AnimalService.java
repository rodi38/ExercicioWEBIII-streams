...
public class AnimalService {
    ...
    public List<Animal> listaDeAnimaisDoTipoNComPesoMaiorQue(List<Animal> animais, int peso, String alimentacao){
            return animais.stream().filter(n -> n.getPeso() > peso &&
                    n.getAlimentacao().equalsIgnoreCase(alimentacao)).toList();
    }
    public List<Animal> listaDeAnimaisDoTipoNComPesoMenorQue(List<Animal> animais, int peso, String alimentacao){
        return animais.stream().filter(n -> n.getPeso() < peso &&
                n.getAlimentacao().equalsIgnoreCase(alimentacao)).toList();
    }
}
